...
class Node:
    ...
    def find_block_to_back_up(self):
        ...
        for block_id, (held_locally, peer) in enumerate(zip(self.local_blocks, self.backed_up_blocks)):
           # held_locally - our block is on our node
           # peer - the block is backed up on a remote node
...
class Online(NodeEvent):
    ...
    def process(self, sim: Backup):
        ...
        node.schedule_next_upload(sim) # class Node
        node.schedule_next_download(sim) # class Node
